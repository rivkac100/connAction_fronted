...
export const Customers = () => {
 ...
 const exportToPDF = async () => {
  try {
    ...
    setSnackbar({
      open: true,
      message: 'מייצא לקוחות ל-PDF, אנא המתן...',
      severity: 'info'
    });

    // יצירת טבלה חדשה שתכיל את כל הלקוחות
    const fullTableContainer = document.createElement('div');
    fullTableContainer.style.width = '100%';
    fullTableContainer.style.direction = 'rtl';
    fullTableContainer.style.fontFamily = 'Arial, sans-serif';
    fullTableContainer.style.position = 'absolute';
    fullTableContainer.style.left = '-9999px';
    fullTableContainer.style.top = '-9999px';
    fullTableContainer.style.backgroundColor = '#ffffff';
    fullTableContainer.style.padding = '20px';
    ...
    headerDiv.innerHTML = `
      <h1 style="color: #b60557; font-size: 24px; margin-bottom: 10px;">רשימת לקוחות</h1>
      <p style="font-size: 14px; color: #666;">הופק בתאריך: ${new Date().toLocaleDateString('he-IL')}</p>
    `;
    fullTableContainer.appendChild(headerDiv);
    ...
    thead.innerHTML = `
      <tr style="background-color: #f5f5f5; border-bottom: 2px solid #ddd;">
        <th style="padding: 12px; text-align: right; font-weight: bold;">קוד לקוח</th>
        <th style="padding: 12px; text-align: right; font-weight: bold;">שם מוסד</th>
        <th style="padding: 12px; text-align: right; font-weight: bold;">טלפון</th>
        <th style="padding: 12px; text-align: right; font-weight: bold;">אימייל</th>
        <th style="padding: 12px; text-align: right; font-weight: bold;">פקס</th>
        <th style="padding: 12px; text-align: right; font-weight: bold;">איש קשר</th>
        <th style="padding: 12px; text-align: right; font-weight: bold;">טלפון איש קשר</th>
        <th style="padding: 12px; text-align: right; font-weight: bold;">עיר</th>
        <th style="padding: 12px; text-align: right; font-weight: bold;">קהילה</th>
      </tr>
    `;
    table.appendChild(thead);
    
    // יצירת גוף הטבלה עם כל הלקוחות
    const tbody = document.createElement('tbody');
    
    // השתמש ב-filteredCustomers שאמור לכלול את כל הלקוחות המסוננים
    filteredCustomers.forEach((customer, index) => {
      ...
      tr.innerHTML = `
        <td style="padding: 12px; text-align: right;">${customer.instituteId || ''}</td>
        <td style="padding: 12px; text-align: right;">${customer.instituteName || ''}</td>
        <td style="padding: 12px; text-align: right;">${customer.mobile || ''}</td>
        <td style="padding: 12px; text-align: right;">${customer.email || ''}</td>
        <td style="padding: 12px; text-align: right;">${customer.fax || ''}</td>
        <td style="padding: 12px; text-align: right;">${customer.contactName || ''}</td>
        <td style="padding: 12px; text-align: right;">${customer.contactPhone || ''}</td>
        <td style="padding: 12px; text-align: right;">${customer.city || ''}</td>
        <td style="padding: 12px; text-align: right;">${customer.community || ''}</td>
      `;
    ...
    fullTableContainer.appendChild(table);
    ...
    footerDiv.innerHTML = `
      <p style="font-size: 12px; color: #666;">סה"כ ${filteredCustomers.length} לקוחות</p>
    `;
    fullTableContainer.appendChild(footerDiv);
    
    // הוספת המיכל לדף
    document.body.appendChild(fullTableContainer);
    
    // המרת המיכל לתמונה
    const canvas = await html2canvas(fullTableContainer, {
      scale: 2,
      useCORS: true,
      logging: false,
    ...
    // הסרת המיכל הזמני
    document.body.removeChild(fullTableContainer);
    ...
    const imgWidth = 297; // רוחב דף A4 במצב לרוחב
    const imgHeight = canvas.height * imgWidth / canvas.width;
    
    // בדיקה אם התמונה גדולה מדי לעמוד אחד
    const pageHeight = 210; // גובה דף A4 במצב לרוחב
    
    // אם התמונה גדולה מעמוד אחד, חלק אותה לעמודים
    if (imgHeight > pageHeight) {
      ...
      // אם התמונה נכנסת בעמוד אחד
      pdf.addImage(canvas.toDataURL('image/png'), 'PNG', 0, 0, imgWidth, imgHeight);
    }
    
    // שמירת ה-PDF
    pdf.save(`רשימת_לקוחות_${new Date().toLocaleDateString('he-IL').replace(/\//g, '-')}.pdf`);
    
    setSnackbar({
      open: true,
      message: 'הייצוא ל-PDF הושלם בהצלחה!',
      severity: 'success'
    });
  } catch (error) {
    ...
    setSnackbar({
      open: true,
      message: 'אירעה שגיאה בייצוא ל-PDF: ' + error.message,
      severity: 'error'
    });
  } finally {
    setLoading(false);
